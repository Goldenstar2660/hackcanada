...
description: Research on repository boundaries, runtime ownership, and tooling for the Binsight mixed web backend device codebase
...
* Best project foundation boundaries for a mixed web, backend, Raspberry Pi, and ESP8266 repository matching the Binsight spec
...
The repo should treat Binsight as four cooperating surfaces with one narrow local
