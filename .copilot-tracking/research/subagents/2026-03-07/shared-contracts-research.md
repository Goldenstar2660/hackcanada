...
description: Research on canonical shared contracts strategy for BinBuddy across web, backend, Raspberry Pi, and ESP8266 surfaces
...
Research the best shared contracts strategy for BinBuddy using the project spec as
the source of truth.
...
Key product and system constraints extracted from `/spec/binbuddy-spec.md`:
...
* Many BinBuddy contracts are not primarily HTTP contracts, including Firestore document shapes,
