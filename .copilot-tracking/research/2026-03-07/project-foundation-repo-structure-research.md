...
Research the best approach for implementing the project foundation, shared contracts, and repo structure for BinBuddy, a smart waste-sorting station with a dashboard website, Raspberry Pi station runtime, ESP8266 firmware, and Firebase-backed data services.
...
* /home/handwash/Projects/hackcanada/spec/binbuddy-spec.md
  * [Demo scope and device UX surfaces](../../../spec/binbuddy-spec.md#L8-L16) require one station runtime, one dashboard surface, one camera flow, LEDs, and LCD feedback.
  * [Classification, local rules usage, and fallback behavior](../../../spec/binbuddy-spec.md#L21-L27) require the Pi runtime to make live disposal decisions from a local rules preset even when cloud services are not in the loop.
  * [Event fields, metrics, and dashboard requirements](../../../spec/binbuddy-spec.md#L43-L104) require shared contracts for events, station status, metadata, and analytics read models.
  * [Hardware split and Firebase notes](../../../spec/binbuddy-spec.md#L124-L145) explicitly separate ESP8266 firmware from Raspberry Pi runtime and place Firestore and Cloud Functions on the cloud side.
...
### Monorepo Foundation for BinBuddy

BinBuddy should start as a single monorepo with surface-first top-level folders and a small set of shared packages. This gives the team one source of truth for product rules, event contracts, and analytics semantics while keeping runtime-specific toolchains independent.
